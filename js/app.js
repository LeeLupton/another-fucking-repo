...
(function () {
  ...
  /** The capture form holds something: typing, a photo, or fields a prefill filled in. The same three things the "understood" card is shown for. */
  function captureHasWork() {
    const cap = state.capture;
    return !!(cap && (cap.dirty || cap.receiptBlob || (cap.text && cap.text.trim())));
  }
  ...
  /** Ask before the capture form alone is replaced. A prefill leaves a recording and a measured trip where they are, so it must not offer to discard them. */
  async function confirmDiscardCapture(what) {
    if (!captureHasWork()) return true;
    const photo = !!(state.capture && state.capture.receiptBlob);
    return confirmDialog('Throw away what you have entered?', `The details you have typed${photo ? ', including the receipt photo you just took,' : ''} will be lost. ${what}`, 'Discard', true);
  }
  ...
  async function prefillCapture(entry) {
    ...
    if (!(await confirmDiscardCapture('The prefilled entry will be shown instead.'))) return false;
  ...
  function bindReceiptRow(panel, entry) {
    ...
    if (rm) rm.onclick = async () => {
      ...
      const sel = panel.querySelector('#eLine');
      row.hidden = S.isMiles(sel ? sel.value : entry.lineId); // a mileage line has nothing left to show, and must not be offered a photo it cannot be saved with
